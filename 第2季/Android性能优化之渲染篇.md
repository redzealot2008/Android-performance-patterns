...
![](http://onmer39jj.bkt.clouddn.com/image/android_performance_course_drop_frame.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/android_performance_course_render_problems.jpg)

## Android UI and the GPU

了解Android是如何利用GPU进行画面渲染有助于我们更好的理解性能问题。一个很直接的问题是：activity的画面是如何绘制到屏幕上的？那些复杂的XML布局文件又是如何能够被识别并绘制出来的？

![](http://onmer39jj.bkt.clouddn.com/image/gpu_rasterization.png)

Resterization栅格化是绘制那些Button，Shape，Path，String，Bitmap等组件最基础的操作。它把那些组件拆分到不同的像素上进行显示。这是一个很费时的操作，GPU的引入就是为了加快栅格化的操作。

CPU负责把UI组件计算成Polygons，Texture纹理，然后交给GPU进行栅格化渲染。

![](http://onmer39jj.bkt.clouddn.com/image/gpu_cpu_rasterization.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/overdraw_hidden_view.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/overdraw_options_view.png)

蓝色，淡绿，淡红，深红代表了4种不同程度的Overdraw情况，我们的目标就是尽量减少红色Overdraw，看到更多的蓝色区域。

## Visualize and Fix Overdraw - Quiz & Solution
...
![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_overdraw_compare.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/overdraw_invisible.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/overdraw_reduce_cpu_gpu.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_clip_1.png)

上面的示例图中显示了一个自定义的View，主要效果是呈现多张重叠的卡片。这个View的onDraw方法如下图所示：

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_clip_3.png)

打开开发者选项中的显示过度渲染，可以看到我们这个自定义的View部分区域存在着过度绘制。那么是什么原因导致过度绘制的呢？

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_clip_2.png)

## Fixing Overdraw with Canvas API

下面的代码显示了如何通过clipRect来解决自定义View的过度绘制，提高自定义View的绘制性能：

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_clip_code_compare.png)

下面是优化过后的效果：

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_clip_result.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_displaylist_execute.png)

然而如果你修改了View中的某些可见组件的内容，那么之前的DisplayList就无法继续使用了，我们需要重新创建一个DisplayList并重新执行渲染指令更新到屏幕上。

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_displaylist_invalidation.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_displaylist_kick_off.png)
...
![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_hierarchy_1.png)

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_hierarchy_2.png)

下图显示了使用2种不同的写法，在Hierarchy Viewer上呈现出来的性能测试差异：

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_hierarchy_3.png)

## Optimizing Your Layout

下图举例演示了如何优化ListItem的布局，通过RelativeLayout替代旧方案中的嵌套LinearLayout来优化布局。

![](http://onmer39jj.bkt.clouddn.com/image/android_perf_course_hierarchy_4.png)